...
from _ctypes.array import (
    Array,
    array_get_slice_params,
    array_slice_getitem,
    array_slice_setitem,
)
...
def _cast_addr(ptr, obj, tp):
    if not (isinstance(tp, _CDataMeta) and tp._is_pointer_like()):
        raise TypeError("cast() argument 2 must be a pointer type, not %s" % (tp,))
    ...
    if isinstance(obj, int):
        ...
        raise TypeError("cast() argument 1 must be a pointer, not %s" % (type(obj),))
    else:
...
def POINTER(cls):
    try:
        ...
        pass
    if type(cls) is str:
        klass = type(_Pointer)("LP_%s" % cls, (_Pointer,), {})
        klass._type_ = "P"
        _pointer_type_cache[id(klass)] = klass
        return klass
    else:
        name = "LP_%s" % cls.__name__
        klass = type(_Pointer)(name, (_Pointer,), {"_type_": cls})
