        ...
        def __new__(
            ...
            *,
            ...
            if not 1 <= digest_size <= self.MAX_DIGEST_SIZE:
                raise ValueError(
                    "digest_size must be between 1 and %s bytes" % self.MAX_DIGEST_SIZE
                )
            ...
            if salt is not None:
                if len(salt) > self.SALT_SIZE:
                    raise ValueError("maximum salt length is %d bytes" % self.SALT_SIZE)
            ...
            if person:
                if len(person) > _lib.BLAKE_PERSONALBYTES:
                    raise ValueError(
                        "maximum person length is %d bytes" % _lib.BLAKE_PERSONALBYTES
                    )
            ...
            if leaf_size is not None:
                ...
                # endian platforms.
                _lib.store32(_ffi.addressof(self._param, "leaf_length"), leaf_size)
            ...
            if node_offset is not None:
                ...
                if class_name == "blake2s":
                    ...
                    _lib.store48(_lib.addressof_node_offset(self._param), node_offset)
                else:
                    # NB: Simple assignment here would be incorrect on big
                    # endian platforms.
                    _lib.store64(_lib.addressof_node_offset(self._param), node_offset)
            ...
            if not 0 <= inner_size <= _lib.BLAKE_OUTBYTES:
                raise ValueError(
                    "inner_size must be between 0 and is %d" % _lib.BLAKE_OUTBYTES
                )
            ...
            if key:
                if len(key) > _lib.BLAKE_KEYBYTES:
                    raise ValueError(
                        "maximum key length is %d bytes" % _lib.BLAKE_KEYBYTES
                    )
