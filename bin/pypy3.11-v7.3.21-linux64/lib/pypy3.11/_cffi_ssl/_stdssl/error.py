...
from _cffi_ssl._stdssl.utility import (
    _string_from_asn1,
    _str_to_ffi_buffer,
    _str_from_buf,
)
...
def pyssl_error(obj, ret):
    ...
    if obj.ssl != ffi.NULL:
        ...
        if err.ssl == SSL_ERROR_ZERO_RETURN:
            ...
            if e == 0:
                if ret == 0 or obj.socket is None:
                    ...
                    if sys.platform == "win32":
                        if err.ws:
                            return fill_sslerror(
                                obj, OSError, err.ws, os.strerror(err.ws), e
                            )
                    if err.c:
                        ffi.errno = err.c
            ...
            err_reason = lib.ERR_GET_REASON(e)
            reason_str = ERR_CODES_TO_NAMES.get((err_lib, err_reason), None)
            if err_lib == lib.ERR_LIB_SSL and reason_str == "CERTIFICATE_VERIFY_FAILED":
                errtype = SSLCertVerificationError
            if lib.Cryptography_HAS_UNEXPECTED_EOF_WHILE_READING:
                ...
                if (
                    lib.ERR_GET_LIB(e) == lib.ERR_LIB_SSL
                    and err_reason == lib.SSL_R_UNEXPECTED_EOF_WHILE_READING
                ):
                    errval = SSL_ERROR_EOF
                    errtype = SSLEOFError
                    errstr = "EOF occurred in violation of protocol"
    ...
    if obj and errtype is SSLCertVerificationError:
        ...
        if lib.Cryptography_HAS_102_VERIFICATION_ERROR_CODES:
            if verify_code == lib.X509_V_ERR_HOSTNAME_MISMATCH:
                verify_str = (
                    "Hostname mismatch, certificate is not "
                    f"valid for '{obj.server_hostname}'."
                )
            elif verify_code == lib.X509_V_ERR_IP_ADDRESS_MISMATCH:
                verify_str = (
                    "IP address mismatch, certificate is not "
                    f"valid for '{obj.server_hostname}'."
                )
        if not verify_str:
            verify_str = ffi.string(
                lib.X509_verify_cert_error_string(verify_code)
            ).decode()
