...
def enum_certificates(store_name):
    ...
    """
    hStore = lib.CertOpenStore(
        lib.CERT_STORE_PROV_SYSTEM_A,
        0,
        ffi.NULL,
    ...
    try:
        while True:
            ...
            keyusage = parseKeyUsage(
                pCertCtx, lib.CERT_FIND_PROP_ONLY_ENHKEY_USAGE_FLAG
            )
            if keyusage is True:
                keyusage = parseKeyUsage(
                    pCertCtx, lib.CERT_FIND_EXT_ONLY_ENHKEY_USAGE_FLAG
                )
